...
**Versão atual:** v2.12.0 — Build marker `20260601-18500-cfgpath`
...
### 2.16. Monitoramento — rotas de dados genéricos (KML/GeoJSON) [v2.12]

Categoria de dados **independente dos modelos meteorológicos**: *bases de dados* = rotas para arquivos KML/GeoJSON externos (focos de queimadas, boias, estações…). Configuradas na **aba de topo "Base de dados"** do modal de configuração — o modal passou a ter **2 níveis** (`#modalTopTabs`: *Configurar modelos e variáveis* | *Base de dados*; `setCfgTopSection`/`cfgTopSection`). Cada base tem ícone, nome, cor, **Rota** (diretório), **Template Nome Arq.**, **Sufixo** (.kml/.geojson/.json), **Mapa-base** e **"Sempre trazer dados atualizados"** (no-store). Persistência: `localStorage` `sismom_bases_v1`, estado em `basesDados`; CRUD em `renderBasesPane`. Núcleo: `gtBaseUrl`, `gtParseKML` (Placemark→Point/Line/Polygon; a tabela HTML do `<description>` vira campos limpos via `_gtKmlDescToProps`), `gtMonitFetch`, `gtMonitAdd`, `gtMonitFilterData`/`gtMonitApplyFilter`, `gtRenderTreeMonit`.

Cada base aparece no menu **"📡 Monitoramento"** da árvore (seção `data-section-id="monitoramento"`, reordenável via `gtMonitOrder`). Por item: liga/desliga, color picker, **↻ Atualizar** (re-busca no-store via `gtMonitAdd`), remover, grip de ordem, ícone **?** com a URL-fonte (tooltip + clique revela), e dropdown **Mostrar: Todas / Ativas / Inativas** (`base.filtro`).

Seed padrão: **"🔥 Queimadas recentes"** (INPE/Programa Queimadas, `eventos_ativos.kml`). Os focos viram **marcadores de chama vetorial** (`drawPoint`, `style.iconShape='flame'` — desenhada com `arc`+`quadraticCurveTo`, **não emoji**, pois o canvas do Chromium não renderiza emoji colorido): **miolo amarelo** = foco no dia mais recente do dataset (`_ativo`), **miolo cinza** `#80868c` = apagado. Rótulo (município, `_rotulo`) acima do marcador, com dedup. Clicar abre **popup read-only** (`gtOpenMonitFeatureInfo`) com os atributos do INPE (tipo, município, datas, duração, dias sem chuva, total de focos, % desmatamento/vegetação/transição, `Situação`). O filtro Todas/Ativas/Inativas usa `_ativo`; o layer guarda `_allData` (completo) e `layer.data` é o subconjunto filtrado.

**Persistência em arquivo (Electron):** `Exportar` (modal de config) inclui `models`+`bases`+`ui` e grava por padrão em `%APPDATA%/GISELE/configuração/gisele-config.json` (IPC `gisele-config:save`); no início, `gtBootstrapConfigFromFile` auto-importa o arquivo da pasta (**"arquivo manda"**); o painel mostra a pasta (`gisele-config:dir`) + botão **Abrir pasta** (`gisele-config:open`). Em browser/build antigo (sem `window.GISELE_CONFIG`) o Exportar cai no download (`_downloadConfigJson`). Aplicação única via `applyImportedConfig` (Importar e auto-import). **Requer rebuild do Electron** — o bridge `GISELE_CONFIG` vive em `main.js`/`preload.js` (sem cópia na raiz).

Render reutiliza `drawGeoJSON`/`gtExtraLayers` (camadas `isMonit`); hit-test, popup e cor reaproveitam a infra de miscelânea (`gtFindMiscFeatureAtLatLon`, `gtSetMiscLayerColor`). Aditivo: nada do fluxo de modelos/miscelânea muda.
